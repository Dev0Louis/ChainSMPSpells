...
public class SpellTableScreen extends HandledScreen<SpellTableScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        int i = this.x;
        int j = this.y;
        SpellTableScreen.drawTexture(matrices, i, j, 0, 0, this.backgroundWidth, this.backgroundHeight);
        int k = (int)(41.0f * this.scrollAmount);
        SpellTableScreen.drawTexture(matrices, i + 119, j + 15 + k, 176 + (this.shouldScroll() ? 0 : 12), 0, 12, 15);
        int l = this.x + 52;
        int m = this.y + 14;
    ...
    protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            List<SpellRecipe> list = this.handler.getAvailableRecipes();
    ...
    private void renderRecipeBackground(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int scrollOffset) {
        for (int i = this.scrollOffset; i < scrollOffset && i < this.handler.getAvailableRecipeCount(); ++i) {
            ...
            SpellTableScreen.drawTexture(matrices, k, m - 1, 0, n, 16, 18);
        }
    }
